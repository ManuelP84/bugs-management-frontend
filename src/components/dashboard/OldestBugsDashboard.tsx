...
const OldestBugsDashboard = (props: Props) => {
    ...
    const dataToShow = bugs.filter(bug => 
        ["asignado", "reincidente", "rechazado"].includes(bug.lifecycle.toLowerCase())).map(bug => {
        return {
            id: bug.id,
            title: bug.title,
    ...
    return (
        <div className="fluid-container">
            {dataToShow.length > 0 ?
                <div className="row">
                    <h6>Top 10: Defectos más antiguos ordenados por fecha (Project: {dataToShow[0].projectId})</h6>
                </div> : <></>}
            <div className="row">
                ...
                {dataToShow.length > 0 ? <div className="col">
                    {dataToShow.map(bug => {
                        return <div className="rounded border border-danger my-2 p-2" key={bug.id}>
                            ...
                            <b > Título del bug: </b><span>{bug.title}</span>    ---
